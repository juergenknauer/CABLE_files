...
## OzFlux sites
all_sites = 'AdelaideRiver', 'AliceSpringsMulga', 'Calperum', 'CapeTribulation', 'CowBay', 'CumberlandPlain', 'DalyPasture', 'DalyUncleared', 'Dargo', \
...
#start_end_date_df.to_csv('D:/CSIRO/CABLE/OzFLUX_startend_dates.csv')
#start_end_date_df.to_csv('D:/CSIRO/CABLE/OzFLUX_startend_dates.txt')
...
for s,site in enumerate(all_sites):
  ...
  if metfile_present:
    ...
    l6_Ta,l6_Ta_flag,Ta_attr = GetSeries(ds,"Ta")
...
#            valid_days_end = list(np.where(date < datetime.datetime(enddate_orig.year,enddate_orig.month,enddate_orig.day))[0])
    ...
    if len(valid) < 1:
      ...
      if site == 'WallabyCreek':
        ...
        if (site == 'CumberlandPlain' and OzFlux_default == False):
          data_sites_counter = 6
          
        print(data_sites_counter)
        fw = np.array(fcover.iloc[fper_index,data_sites_counter - 1])   # very error prone!! replace counter here!
        fg = np.array(fcover.iloc[frec_index,data_sites_counter - 1])
        ...
        for yr in years:
          ...
          frac_forest = sitelist.loc[site,'forest_fraction']
          frac_grass = sitelist.loc[site,'grass_fraction']
