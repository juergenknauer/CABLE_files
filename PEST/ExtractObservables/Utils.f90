...
interface operator (.eq.)
  module procedure EQTimes
end interface
interface operator (.ne.)
  module procedure NETimes
end interface
!-------------------------------------------------------------------------------
...
!###############################################################################
  ...
  function EQTimes(Time1,Time2)
!-------------------------------------------------------------------------------
! Extends the '.eq.' operator to enable comparison of two times for equality.
!-------------------------------------------------------------------------------
  implicit none
  logical                    :: EQTimes   ! Result of equality test between times
  type (hmtime), intent(in)  :: Time1     ! First time for logical comparison
  type (hmtime), intent(in)  :: Time2     ! Second time for logical comparison
!-------------------------------------------------------------------------------
  EQTimes = .true.
  if (Time1%hour   .ne. Time2%hour   .or. &
      Time1%minute  .ne. Time2%minute) EQTimes = .false.
  end function EQTimes

!###############################################################################

  function NETimes(Time1,Time2)
!-------------------------------------------------------------------------------
! Extends the '.ne.' operator to enable comparison of two times for inequality.
!-------------------------------------------------------------------------------
  implicit none
  logical                    :: NETimes   ! Result of inequality test between times
  type (hmtime), intent(in)  :: Time1     ! First time for logical comparison
  type (hmtime), intent(in)  :: Time2     ! Second time for logical comparison
!-------------------------------------------------------------------------------  
  NETimes = .not. EQTimes(Time1,Time2)
  end function NETimes
